...
from .helpers import summarization_evaluation
from .helpers import text_preprocessing
from .helpers import textblob_sentiment
from .helpers.util.summarization_suggestions_enum import SummarizationSuggestions


def evaluate(answers):
    student_answer_preprocessed = text_preprocessing.preprocess_text_basic(answers.student_answer)
    ratio, status = summarization_evaluation.check_rule_violated(answers.question, student_answer_preprocessed)
    if not status:
        ...
        comprehensiveness_score = basic_metrics.get_comprehensiveness(answers.word_limit,
                                                                      answers.word_count)
        overall_score = summarization_evaluation.calculate_overall_score(spelling_score,
                                                                         grammar_score,
                                                                         similarity_score,
                                                                         objectivity_score,
                                                                         comprehensiveness_score)
        suggestion = summarization_evaluation.get_suggestions(overall_score,
                                                              spelling_score,
                                                              grammar_score,
                                                              similarity_score,
                                                              objectivity_score,
        ...
        return {
            ...
            'overall': 0,
            'ratio': ratio,
            'suggestion': SummarizationSuggestions.COPIED
        }
